...
export default function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    fetch(`/api/products/${id}`)
      ...
      .then((p) => {
        reset({
          ...
          category: p.category ?? "",
          origin: p.origin ?? "",
          materialsRaw: (p.materials ?? []).join(", "),
          packagingType: p.packagingType ?? "",
          hasRecycling: p.hasRecycling ?? false,
          hasOrganicCert: p.hasOrganicCert ?? false,
          isFairTrade: p.isFairTrade ?? false,
          isVegan: p.isVegan ?? false,
          isLocalDelivery: p.isLocalDelivery ?? false,
          hasCarbonNeutral: p.hasCarbonNeutral ?? false,
          hasEnergyEfficiency: p.hasEnergyEfficiency ?? false,
          hasZeroWaste: p.hasZeroWaste ?? false,
  ...
  const addFiles = (files: FileList | null) => {
    ...
    const next = [...newImageFiles, ...arr].slice(0, 8 - existingImages.length);
  ...
  const calcPreview = () => {
    const v = formValues;
    const materials = (v.materialsRaw ?? "").split(",").map((s) => s.trim()).filter(Boolean);
    setPreviewScore(
      calculateEcoScore({
        materials,
        origin: v.origin ?? "",
        hasRecycling: !!v.hasRecycling,
        hasOrganicCert: !!v.hasOrganicCert,
        isFairTrade: !!v.isFairTrade,
        packagingType: v.packagingType ?? "",
        isVegan: !!v.isVegan,
        isLocalDelivery: !!v.isLocalDelivery,
        hasCarbonNeutral: !!v.hasCarbonNeutral,
        hasEnergyEfficiency: !!v.hasEnergyEfficiency,
        hasZeroWaste: !!v.hasZeroWaste,
        isDurable: !!v.isDurable,
      })
    );
  };
  ...
  if (fetching) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center text-sm text-[#6b6b6b]">
        Loading product…
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 sm:px-6 py-10">
      <h1 className="text-2xl font-bold text-[#0a0a0a] mb-2">Edit product</h1>
      <p className="text-sm text-[#6b6b6b] mb-8">
        After saving, the product will go back to review.
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
        ...
        <Textarea id="description" label="Description *" placeholder="Describe the product…" error={errors.description?.message} {...register("description")} />
        ...
        <Input id="packagingType" label="Packaging type" placeholder="Recycled paper" {...register("packagingType")} />
          ...
          {(existingImages.length + newImageFiles.length) < 8 && (
            <>
              ...
              >
                ...
                <span className="text-sm text-[#0a0a0a]">Add more photos</span>
                <span className="text-xs text-[#6b6b6b]">JPG, PNG, WEBP · up to 8 total</span>
              </button>
        ...
        <div className="border border-[#e5e5e5] rounded-xl p-5">
              ...
              return (
                <label
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 cursor-pointer transition-all select-none ${
                    isChecked ? "border-green-500 bg-green-50" : "border-[#e5e5e5] bg-white hover:border-gray-300"
                  }`}
                >
                  ...
                  <span className={`text-xs flex-1 leading-snug ${isChecked ? "text-green-900 font-medium" : "text-[#3a3a3a]"}`}>
                    {attr.label}
                  </span>
                  <span className={`text-xs font-bold shrink-0 ${isChecked ? "text-green-600" : "text-[#b0b0b0]"}`}>
                    {attr.points}
                  </span>
          ...
          {previewScore !== null && (
            <div className="mt-3"><EcoScoreBadge score={previewScore} showLabel /></div>
          )}
        </div>
